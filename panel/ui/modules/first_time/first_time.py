...
class MakeFiles:
    ...
    def __init__(self) -> None:
        ...
        self.directoryName = "Somali-Ware"
    ...
    def fix_key_and_certs(self) -> None:
        ...
        subject = issuer = x509.Name(
            [
                ...
                x509.NameAttribute(
                    NameOID.ORGANIZATIONAL_UNIT_NAME, cert_subject["OU"]
                ),
    ...
    def make_key_pairs(self) -> None:
        """Method to make the key pairs for the application."""
        key = RSA.generate(2048)
        private_key = key.export_key()

        if not os.path.exists(
            os.path.join(self.appdir, self.directoryName, self.keys_directory)
        ):
            os.mkdir(os.path.join(self.appdir, self.directoryName, self.keys_directory))

        with open(
            os.path.join(
                self.appdir, self.directoryName, self.keys_directory, "private.pem"
            ),
            "wb",
        ) as file_out:
        ...
        with open(
            os.path.join(
                self.appdir, self.directoryName, self.keys_directory, "receiver.pem"
            ),
            "wb",
        ) as file_out:
            file_out.write(public_key)

    def get_key_pairs(self) -> str:
        """Method to get the path of the key pairs.

        Returns:
            str: Returns the path of the key pairs.
        """
        return os.path.join(
            self.appdir, self.directoryName, self.keys_directory, "receiver.pem"
        )
    ...
    def ensure_all_dirs(self) -> None:
        ...
        check_pairs = {
            ...
            self.get_key_pairs(): self.make_key_pairs,
        }

        logging.critical(
            r"Ensuring all directories are present! If any are missing, the program will attempt to create them. If this fails, please delete the Somali-Ware folder in %appdata%."
        )
